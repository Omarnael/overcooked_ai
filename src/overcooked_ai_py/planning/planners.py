...
class JointMotionPlanner(object):
    ...
    def is_valid_jm_start_goal_pair(self, joint_start_state, joint_goal_state):
        ...
        return all([check_valid_fn(joint_start_state[i], joint_goal_state[i]) for i in range(1)])
...
class MediumLevelActionManager(object):
    ...
    def get_medium_level_actions(self, state, player, waiting_substitute=False):
        ...
        if not player.has_object():
            ...
            player_actions.extend(projector_pickup + laptop_pickup + solar_cell_pickup + container_pickup + soup_pickup + start_cooking)
...
#         num_projectors_in_partially_full_construction_sites = sum([state.get_object(loc).state[1] for loc in partially_full_soups])
#
#         # Calculating costs
