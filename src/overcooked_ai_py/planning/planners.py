...
class MediumLevelActionManager(object):
    ...
    def pickup_laptop_actions(self, counter_objects, only_use_dispensers=False):
        """If only_use_dispensers is True, then only take laptops from the dispensers"""
        laptop_pickup_locations = self.mdp.get_laptop_dispenser_locations()
        if not only_use_dispensers:
            laptop_pickup_locations += counter_objects['laptop']
...
#         # NOTE: does not support solar_cells – currently deprecated as harder heuristic
