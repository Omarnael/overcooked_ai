...
class Recipe:
    ...
    laptop = 'laptop'
    ALL_INGREDIENTS = [laptop, solar_cell]

    ALL_RECIPES_CACHE = {}
    STR_REP = {'solar_cell': "†", 'laptop': "ø"}
    ...
    def __int__(self):
        ...
        mixed_mask = int(bool(num_solar_cells * num_laptops))
        mixed_shift = (Recipe.MAX_NUM_INGREDIENTS + 1)**len(Recipe.ALL_INGREDIENTS)
        encoding = num_laptops + (Recipe.MAX_NUM_INGREDIENTS + 1) * num_solar_cells
    ...
    def value(self):
        ...
        if self._laptop_value and self._solar_cell_value:
            ...
            return self._solar_cell_value * num_solar_cells + self._laptop_value * num_laptops
        return 20
    ...
    def time(self):
        ...
        if self._laptop_time and self._solar_cell_time:
            num_laptops = len([ingredient for ingredient in self.ingredients if ingredient == self.laptop])
            num_solar_cells = len([ingredient for ingredient in self.ingredients if ingredient == self.solar_cell])
            return self._laptop_time * num_laptops + self._solar_cell_time * num_solar_cells
        return 20
    ...
    def configure(cls, conf):
        ...
        # Mutual Exclusion
        if 'solar_cell_time' in conf and not 'laptop_time' in conf or 'laptop_time' in conf and not 'solar_cell_time' in conf:
            raise ValueError("Must specify both 'laptop_time' and 'solar_cell_time'")
        if 'solar_cell_value' in conf and not 'laptop_value' in conf or 'laptop_value' in conf and not 'solar_cell_value' in conf:
            raise ValueError("Must specify both 'laptop_value' and 'solar_cell_value'")
        if 'solar_cell_value' in conf and 'delivery_reward' in conf:
            raise ValueError("'delivery_reward' incompatible with '<ingredient>_value'")
        if 'solar_cell_value' in conf and 'recipe_values' in conf:
...
class SoupState(ObjectState):
    ...
    def from_dict(cls, obj_dict):
        ...
        if 'state' in obj_dict:
            ...
            finished = time >= 20
            if ingredient == Recipe.solar_cell:
    ...
    @classmethod
    def get_soup(cls, position, num_laptops=1, num_solar_cells=0, cooking_tick=-1, finished=False, **kwargs):
        if num_laptops < 0 or num_solar_cells < 0:
            raise ValueError("Number of active ingredients must be positive")
        if num_laptops + num_solar_cells > Recipe.MAX_NUM_INGREDIENTS:
            raise ValueError("Too many ingredients specified for this soup")
        if cooking_tick >= 0 and num_solar_cells + num_laptops == 0:
            raise ValueError("_cooking_tick must be -1 for empty soup")
        if finished and num_solar_cells + num_laptops == 0:
        ...
        ingredients = laptops + solar_cells
        ...
        def start_state_fn():
            ...
            for player in start_state.players:
                ...
                if p < rnd_obj_prob_thresh:
                    ...
                    if obj == "soup":
                        player.set_object(
                            SoupState.get_soup(player.position, num_laptops=n, num_solar_cells=m, finished=True)
                        )
                    else:
    ...
    def resolve_interacts(self, new_state, joint_action, events_infos):
        ...
        for player_idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                ...
                # solar_cell pickup from dispenser
    ...
    def lossless_state_encoding(self, overcooked_state, horizon=400, debug=False):
        ...
        def process_for_player(primary_agent_idx):
            ...
            for obj in all_objects:
                if obj.name == "soup":
                    # removed the next line because laptop doesn't have to be in all the soups?
                    # if Recipe.laptop in obj.ingredients:
    ...
    def potential_function(self, state, mp, gamma=0.99):
        ...
        for soup in idle_soups:
            ...
            for ingredient in missing_ingredients:
                ...
                pertinent_players = players_holding_solar_cells if ingredient == Recipe.solar_cell else players_holding_laptops
                dist = np.inf
                closest_player = None
