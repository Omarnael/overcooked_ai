...
import random, copy
from hacktrick_ai_py.utils import rnd_int_uniform, rnd_uniform
from hacktrick_ai_py.mdp.actions import Action, Direction
from hacktrick_ai_py.mdp.hacktrick_mdp import HacktrickGridworld, Recipe
...
CONSTRUCTION_SITE = 'P'
CONTAINER_DISPENSER = 'D'
SERVING_LOC = 'S'

CODE_TO_TYPE = {0: EMPTY, 1: COUNTER, 2: PROJECTOR_DISPENSER, 3: LAPTOP_DISPENSER, 4: CONSTRUCTION_SITE, 5: CONTAINER_DISPENSER,
...
DEFAULT_FEATURE_TYPES = (CONSTRUCTION_SITE, PROJECTOR_DISPENSER, CONTAINER_DISPENSER, SERVING_LOC, LAPTOP_DISPENSER, SOLAR_CELL_DISPENSER) # NOTE: TOMATO_DISPENSER is disabled by default
...
class LayoutGenerator(object):
    ...
    def generate_padded_mdp(self, outside_information={}):
        ...
        if "layout_name" in mdp_gen_params.keys() and mdp_gen_params["layout_name"] is not None:
            mdp = HacktrickGridworld.from_layout_name(**mdp_gen_params)
    ...
    def padded_mdp(self, mdp, display=False):
        ...
        return HacktrickGridworld.from_grid(mdp_grid)
    ...
    def make_disjoint_sets_layout(self, inner_shape, prop_empty, prop_features, base_param, feature_types=DEFAULT_FEATURE_TYPES, display=True):        
        ...
        start_positions = self.get_random_starting_positions(padded_grid)
        mdp_grid = self.padded_grid_to_layout_grid(padded_grid, start_positions, display=display)
        return HacktrickGridworld.from_grid(mdp_grid, base_param)
    ...
    def padded_grid_to_layout_grid(self, padded_grid, start_positions, display=False):
        ...
        # Start formatting to actual HacktrickGridworld input type
