...
from hacktrick_ai_py.utils import save_pickle, load_pickle, cumulative_rewards_from_rew_list, save_as_json, \
    load_from_json, merge_dictionaries, rm_idx_from_dict, take_indexes_from_dict, is_iterable
from hacktrick_ai_py.planning.planners import NO_COUNTERS_PARAMS
from hacktrick_ai_py.agents.agent import AgentPair, RandomAgent, GreedyHumanModel
from hacktrick_ai_py.mdp.hacktrick_mdp import HacktrickGridworld, Action, HacktrickState
from hacktrick_ai_py.mdp.hacktrick_env import HacktrickEnv
from hacktrick_ai_py.mdp.layout_generator import LayoutGenerator
from hacktrick_ai_py.mdp.hacktrick_trajectory import DEFAULT_TRAJ_KEYS
...
class AgentEvaluator(object):
    ...
    def __init__(self, env_params, mdp_fn, force_compute=False, mlam_params=NO_COUNTERS_PARAMS, debug=False):
        """
        env_params (dict): params for creation of an HacktrickEnv
        ...
        self.mdp_fn = mdp_fn
        self.env = HacktrickEnv(self.mdp_fn, **env_params)
    ...
    def from_mdp_params_infinite(mdp_params, env_params, outer_shape=None, mdp_params_schedule_fn=None, force_compute=False,
        ...
        """
        mdp_params (dict): params for creation of an HacktrickGridworld instance through the `from_layout_name` method
    ...
    def from_mdp(mdp, env_params, force_compute=False, mlam_params=NO_COUNTERS_PARAMS, debug=False):
        """
        mdp (HacktrickGridworld): the mdp that we want the AgentEvaluator to always generate
        Information for the rest of params please refer to the __init__ method above
        """
        assert type(mdp) == HacktrickGridworld, "mdp must be a HacktrickGridworld object"
    ...
    def from_mdp_lst(mdp_lst, env_params, sampling_freq=None, force_compute=False, mlam_params=NO_COUNTERS_PARAMS, debug=False):
        """
        mdp_lst (list): a list of mdp (HacktrickGridworld) we would like to
        ...
        assert all([type(mdp) == HacktrickGridworld for mdp in mdp_lst]), "some mdps are not HacktrickGridworld objects"
    ...
    def _check_right_types(trajectories):
        for idx in range(len(trajectories["ep_states"])):
            ...
            assert all(type(s) is HacktrickState for s in states)
    ...
    def get_mdps_and_envs_from_trajectories(trajectories):
        ...
        for idx in range(len(trajectories["ep_lengths"])):
            ...
            mdp = HacktrickGridworld.from_layout_name(**mdp_params)
            env = HacktrickEnv.from_mdp(mdp, **env_params)
            mdps.append(mdp)
            envs.append(env)
        return mdps, envs
    ...
    def load_traj_from_json(filename):
        ...
        traj_dict["ep_states"] = [[HacktrickState.from_dict(ob) for ob in curr_ep_obs] for curr_ep_obs in traj_dict["ep_states"]]
