...
class NNPolicy(object):
    ...
    def multi_state_policy(self, states, agent_indices):
        """
        A function that takes in multiple HacktrickState instances and their respective agent indices and returns action probabilities.
        """
        raise NotImplementedError()

    def multi_obs_policy(self, states):
        """
        A function that takes in multiple preprocessed HacktrickState instatences and returns action probabilities.
        """
        raise NotImplementedError()
    ...
    def ml_action(self, state):
        ...
        if not player.has_object():
            ...
            if solarlab_nearly_ready and not other_has_container:
                ...
                #     motion_goals = am.pickup_laptop_actions(counter_objects)
                # else:
                #     motion_goals = am.pickup_projector_actions(counter_objects) + am.pickup_laptop_actions(counter_objects)


        else:
            player_obj = player.get_object()

            if player_obj.name == 'projector':
                motion_goals = am.put_projector_in_construction_site_actions(construction_site_states_dict)

            elif player_obj.name == 'laptop':
